...
<div class="grid gap-6 mb-8 md:grid-cols-2">
    <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
        ...
        <ul>
            <li>ID: {{ $refugio->id }}</li>
            <li>Dirección: {{ $refugio->Direccion }}</li>
            <li>Cantidad de Perros: {{ $refugio->CantidadPerros }}</li>
        </ul>
        </p>
    </div>
    <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
        <h4 class="mb-4 font-semibold text-gray-600 dark:text-gray-300">
            Voluntarios en el refugio
        </h4>
        <p class="text-gray-600 dark:text-gray-400">
            <ul>
                @foreach($refugio->voluntarios as $voluntario)
    ...
    <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
        ...
        <p class="text-gray-600 dark:text-gray-400">
        <form action="{{ route('refugio.agrega-voluntario', $refugio) }}" method="POST">
            @csrf
            <label class="block mt-4 text-sm">
                <span class="text-gray-700 dark:text-gray-400">
                    Seleccione Voluntario
                </span>
                <select
                    ...
                    @foreach($voluntarios as $voluntario)
                        <option name="{{ $voluntario->id }}" value="{{ $voluntario->id }}">{{ $voluntario->Nombre }}</option>
                    @endforeach
                </select>
            </label>
